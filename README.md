...
**RAG / Vector Indexer (Troubleshooting)**

- When running the backend locally (not via Docker), the vector indexer is typically available on `http://localhost:8001` (the project maps container port `8000` to host `8001`). If you see 500s on `POST /api/rag-search`, set `VECTOR_INDEXER_URL=http://localhost:8001` in your environment or start the vector indexer container with `docker compose up pathway-vector-indexer`.
- The backend now falls back to `http://localhost:8001` automatically if the configured `VECTOR_INDEXER_URL` is unreachable.
