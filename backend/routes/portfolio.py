...
async def upload_portfolio_csv(
    ...
    """Upload portfolio holdings from CSV - FIXED"""
    portfolio = portfolios_db.get(portfolio_id)
    ...
    try:
        ...
        for col in df.columns:
            if 'symbol' in col or 'ticker' in col:
                ...
                quantity_col = col
            elif 'cost' in col or 'price' in col or 'avg' in col:
                cost_col = col
        
        if not all([symbol_col, quantity_col, cost_col]):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"CSV must contain: symbol, quantity, and cost columns. Found: {list(df.columns)}"
            )
        ...
        if not new_holdings:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"No valid holdings found. Errors: {errors}"
            )
        ...
        return {
            "message": f"Successfully imported {len(new_holdings)} holdings",
            "total_invested": total_invested,
            "total_value": portfolio["total_value"],
